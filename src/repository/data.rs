...
pub async fn getbydevice(_device:String)->Result<Vec<Data>,tokio_postgres::Error>{
    let (client, connection) =
        tokio_postgres::connect(DB::url().url, NoTls).await?;
...
pub async fn insert(p : Data)->Result<u64,tokio_postgres::Error>{
    ...
    let command = format!("with i as (
                                        update datas set date = $3 , value = $2 where device = $1 and exists (select 1 from datas where device = $1)
                                    ),  u as (
                                        insert into datas (device,value,date) 
                                            SELECT $1,$2,$3
